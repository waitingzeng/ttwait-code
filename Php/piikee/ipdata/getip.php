...
//取得客户端ip并判断地址
   ...
   $myobj=NULL;
   $str="<span class='orange'>".$ip."</span>&nbsp;&nbsp;来自:".$address["area1"]." ".$address["area2"]."<br>";
   echo $str;
//处理字符串，以得到能与数据库里面相匹配的字条  
$prince=explode("省",$address["area1"]);
  if(!empty($prince[1]))
{
  $city=explode("市",$prince[1]);
  $yourcity=$city[0]."市";
}
  elseif(empty($prince[1]))
{
	       $city2=array();
	       $city2[]=strpos($address["area1"],"上海");
	       $city2[]=strpos($address["area1"],"北京");
		   $city2[]=strpos($address["area1"],"天津");
		   $city2[]=strpos($address["area1"],"重庆");
	       $city4=array();
		   $city4[]="上海";
		   $city4[]="北京";
		   $city4[]="天津";
	   ...
	   if(empty($yourcity)){
	       $city1=array();
	       $city1[]=explode("内蒙古",$address["area1"]);
	       $city1[]=explode("宁夏",$address["area1"]);
		   $city1[]=explode("广西",$address["area1"]);
		   $city1[]=explode("西藏",$address["area1"]);
		   $city1[]=explode("新疆",$address["area1"]);
          for($j=0;$j<=4;$j++){
	      if(!empty($city1[$j][1])){
		  $city3=$city1[$j][1];
            }
          }
	$city=explode("市",$city3);        
    $yourcity=$city[0];
        }
}
if(!empty($yourcity)) echo $yourcity;
if(empty($yourcity)) echo"不知道你的地址了";
