...
function install($file,$dbname){
    ...
    $cfg_dbname=$dbname;
    $db->connect($cfg_dbhost,$cfg_dbuser,$cfg_dbpwd ,$cfg_dbname,''); 
	...
	foreach($datas as $v)
	{
	if(strlen($v)>10) $db->query("{$v}");
	}
	return 1;
}


function install1($file){
    global $db;
    include "../config/common.inc.php";  
    $cfg_dbname=$dbname;
    $db->connect($cfg_dbhost,$cfg_dbuser,$cfg_dbpwd ,$cfg_dbname,''); 
	...
	$fp = fopen($file, "r");
	$data = fread($fp, filesize($file));
	$data = stripslashes(iconv("utf-8","gb2312",$data));
...
function sqls($files)
{
	global $dsql;
	$fp = fopen($files, "r");
	$sqlquery = fread($fp, filesize($files));
...
/* 
函数称号：inject_check() 
函数做用：检测降接的值是没有是露有SQL打针的字符，避免打针，维护效劳器平安 
参　　数：$sql_str: 提交的变质 
往 归 值：前往检测成果,十月妈咪，ture or false 
*/ 
...
$id=intval($id);
if (!$id) { exit('没有提交参数！'); } // 能否为空判别 
elseif (inject_check($id)) { exit('提交的参数非法！'); } // 打针断定 
elseif (!is_numeric($id)) { exit('提交的参数非法！'); } // 数字断定 
$id = intval($id); // 整型化 
...
if (!get_magic_quotes_gpc()) { // 断定magic_quotes_gpc能否翻开 
$str = addslashes($str); // 入止功滤 
} 
$str = str_replace("_", "\_", $str); // 把 '_'过滤掉 
$str = str_replace("%", "\%", $str); // 把 '%'功滤掉 

return $str; 
} 

function post_check($post) { 
if (!get_magic_quotes_gpc()) { // 判别magic_quotes_gpc是否为翻开 
$post = addslashes($post); // 入止magic_quotes_gpc出有翻开的情形对于降接数据的功滤 
} 
$post = str_replace("_","\_", $post); // 把 '_'过滤掉 
$post = str_replace("%","\%", $post); // 把 '%'过滤掉 
$post = nl2br($post); // 回车转换 
$post = htmlspecialchars($post); // html标志转换 
