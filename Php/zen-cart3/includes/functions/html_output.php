...
#TTwait Add
function zen_image_my($src, $alt = '', $width = '', $height = '', $parameters = '') {
	
	$image = '<img src="' . zen_output_string($src) . '" alt="' . zen_output_string($alt) . '"';
	if(zen_not_null($width)){
    	$image .= ' width="' . $width .'"';
	} 
    if(zen_not_null($height)){
     	$image .= ' height="' . $height . '"';
    }
   // $image .= ' onerror="this.src=\''. DIR_WS_IMAGES . PRODUCTS_IMAGE_NO_IMAGE .'\';this.onerror=\'\'"';
 ...
 */
  function zen_image($src, $alt = '', $width = '', $height = '', $parameters = '') {
    ...
    #TTwait Add 
    if (strstr($src, DIR_WS_IMAGES.'pic/')){
    	return zen_image_my($src, $alt, $width, $height, $parameters);
    }
    #TTwait Add End
