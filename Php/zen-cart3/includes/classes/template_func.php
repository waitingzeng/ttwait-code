...
class template_func extends base {
  ...
  function get_template_dir($template_code, $current_template, $current_page, $template_dir, $debug=false) {
    ...
    #TTwait Add 
    elseif(SITE_DOAMIN_KEY && $this->file_exists($current_template . $template_dir . '/' . SITE_DOMAIN_KEY, preg_replace('/\//', '', $template_code), $debug)) {
    	return $current_template . $template_dir . '/' . SITE_DOMAIN_KEY;
    }
    #TTwait Add End
