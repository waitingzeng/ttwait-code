...
class order extends base {
  ...
  function query($order_id) {
    ...
    $order_query = "select customers_id, customers_name, customers_company,
                        ...
                        from " . TABLE_ORDERS . "
                        where orders_id = '" . $order_id . "'"; #TTwait Change
    ...
    $totals_query = "select title, text, class
                         ...
                         order by sort_order";#TTwait Change
    ...
    $order_total_query = "select text, value
                             from " . TABLE_ORDERS_TOTAL . "
                             where orders_id = '" . $order_id . "'
                             and class = 'ot_total'"; #TTwait Change
    ...
    $shipping_method_query = "select title, value
                                from " . TABLE_ORDERS_TOTAL . "
                                where orders_id = '" . $order_id . "'
                                and class = 'ot_shipping'"; #TTwait Change
    ...
    $index = 0;
    $orders_products_query = "select orders_products_id, products_id, products_name,products_image,
                                 ...
                                 products_discount_type_from
                                  ...
                                  where orders_id = '" . $order_id . "'
                                  order by orders_products_id"; #TTwait Change
    ...
    while (!$orders_products->EOF) {
      ...
      $this->products[$index]['link'] = zen_href_link(zen_get_info_page($id), 'products_id=' . $id);
      $this->products[$index]['image'] = (IMAGE_SHOPPING_CART_STATUS == 1 ? zen_image(DIR_WS_IMAGES . $orders_products->fields['products_image'], $orders_products->fields['products_name'], IMAGE_SHOPPING_CART_WIDTH, IMAGE_SHOPPING_CART_HEIGHT) : '');
      ...
      $attributes_query = "select products_options_id, products_options_values_id, products_options, products_options_values,
                              options_values_price, price_prefix from " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . "
                               where orders_id = '" . $order_id . "'
                               and orders_products_id = '" . (int)$orders_products->fields['orders_products_id'] . "'"; #TTwait Change
...
*/
    ...
    #TTwait add
	$products_options_name_1 = '';
	$products_sizelist = array();
	foreach ($products as $values) {
		$products_id = explode(':', $values['id']);
		$products_sizelist[$products_id[0]] = 1;
	}
	if(count($products_sizelist) > 0){
		$sql = 'select products_id,products_sizelist from '. TABLE_PRODUCTS . ' where products_id  in ('. join(',', array_keys($products_sizelist)) .')';
		$sizelist_values = $db->Execute($sql);
		while (!$sizelist_values->EOF) {
			$id = $sizelist_values->fields['products_id'];
			$sizelist = explode(';', $sizelist_values->fields['products_sizelist']);
			$products_sizelist[$id] = $sizelist;
			$sizelist_values->MoveNext();
		}
	}
	#TTwait end
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      #TTwait add
      $products_id = explode(':', $products[$i]['id']);
      ...
      $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                      ...
                                      'id' => $products[$i]['id'],
                                      'link' => zen_href_link(zen_get_info_page($products[$i]['id']), 'products_id=' . $products[$i]['id']),
                                      'imageRaw' => $products[$i]['image'],
                                      'image' => (IMAGE_SHOPPING_CART_STATUS == 1 ? zen_image(DIR_WS_IMAGES . $products[$i]['image'], $products[$i]['model'], IMAGE_SHOPPING_CART_WIDTH, IMAGE_SHOPPING_CART_HEIGHT) : ''),
                                      'rowClass' => $rowClass);
      ...
      if ($products[$i]['attributes']) {
        ...
        #TTwait add
	      if($option == 1){
	      	if($products_options_name_1 == ''){
		      	$sql = "select products_options_name  from " . TABLE_PRODUCTS_OPTIONS . " popt
		              where popt.products_options_id='1' 
		              and popt.language_id = '" . (int)$_SESSION['languages_id'] . "' limit 1" ;
		      	$attributes = $db->Execute($sql);
		      	$products_options_name_1 = $attributes->fields['products_options_name'];
	      	}
	      	$this->products[$index]['attributes'][$subindex] = array('option' => $products_options_name_1,
                                                                   'value' => $products_sizelist[$products_id][$value],
                                                                   'option_id' => $option,
                                                                   'value_id' => $value,
                                                                   'prefix' => 0,
                                                                   'price' => 0);
  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      $sql_data_array = array('orders_id' => $zf_insert_id,
                              ...
                              'products_image' => $this->products[$i]['imageRaw'],
      ...
      if (isset($this->products[$i]['attributes'])) {
        ...
        for ($j=0, $n2=sizeof($this->products[$i]['attributes']); $j<$n2; $j++) {
          if($this->products[$i]['attributes'][$j]['option_id'] == '1'){
          	$attributes_query = "select products_options_name  from " . TABLE_PRODUCTS_OPTIONS . " popt
              where popt.products_options_id='1' 
              and popt.language_id = '" . (int)$_SESSION['languages_id'] . "' limit 1" ;
