...
function drawFiles($list, &$manager)
{
	...
	{ ?>
		...
		<a href="#" onClick="selectImage('<? echo $file['relative'];?>', '<? echo $entry; ?>', <? echo $file['image'][0];?>, <? echo $file['image'][1]; ?>);"title="<? echo $entry; ?> - <? echo Files::formatSize($file['stat']['size']); ?>"><img src="<? echo $manager->getThumbnail($file['relative']); ?>" alt="<? echo $entry; ?> - <? echo Files::formatSize($file['stat']['size']); ?>" width='80' height='80' /></a>
		</td></tr><tr><td class="edit">
			<a href="images.php?dir=<? echo $relative; ?>&amp;delf=<? echo rawurlencode($file['relative']);?>&b=<?=$_GET[b]?>&p=<?=$_GET[p]?>" title="Trash" onClick="return confirmDeleteFile('<? echo $entry; ?>');"><img src="img/edit_trash.gif" height="15" width="15" alt="Trash"/></a><a href="javascript:;" title="Edit" onClick="editImage('<? echo rawurlencode($file['relative']);?>','<?=$_GET[b]?>','<?=$_GET[p]?>');"><img src="img/edit_pencil.gif" height="15" width="15" alt="Edit"/></a>
		<? if($file['image']){ echo $file['image'][0].'x'.$file['image'][1]; } else echo substr($entry,0,5)."...";?>
		</td></tr></table></td> 
	  <? 
	}//foreach
}//function drawFiles
...
function drawDirs($list, &$manager) 
{
	...
	{ ?>
		<td><table width="100" cellpadding="0" cellspacing="0"><tr><td class="block">
		<a href="images.php?dir=<? echo rawurlencode($path); ?>&b=<?=$_GET[b]?>&p=<?=$_GET[p]?>" onClick="updateDir('<? echo $path; ?>&b=<?=$_GET[b]?>&p=<?=$_GET[p]?>')" title="<? echo $dir['entry']; ?>"><img src="img/folder.gif" height="80" width="80" alt="<? echo $dir['entry']; ?>" /></a>
		</td></tr><tr>
		<td class="edit">
			<a href="images.php?dir=<? echo $relative; ?>&amp;deld=<? echo rawurlencode($path); ?>&b=<?=$_GET[b]?>&p=<?=$_GET[p]?>" title="Trash" onClick="return confirmDeleteDir('<? echo $dir['entry']; ?>', <? echo $dir['count']; ?>);"><img src="img/edit_trash.gif" height="15" width="15" alt="Trash"/></a>
			<? echo $dir['entry']; ?>
		</td>
		</tr></table></td>
	  <? 
...
<? } ?>
	...
	function editImage(image,b,p) 
	{
		var url = "editor.php?img="+image+"&b="+b+"&p="+p;
...
<? } else { drawNoResults(); } ?>
<?
	$size = $manager->dir_size($_GET[b], false);
	echo "<span class='edit'>&nbsp;&nbsp;Total: ".round(($size/1024/1024),2)." MB ";
	echo "<strong>(".round(($IMConfig['dir_max_size']/1024/1024),2)." MB max.)</strong></span>\n";
?>
</body>
</html>
