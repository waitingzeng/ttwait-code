...
class ImageManager 
{
	...
	function processUploads() 
	{
		...
		{
			if($_FILES['upload']['size']<=$this->config['max_size'])
			{
				if($this->dir_size($this->getBaseDir())+$_FILES['upload']['size']<=$this->config['dir_max_size'])
				{
	...
	function getThumbnail($relative) 
	{
		...
		Return 'thumbs.php?img='.rawurlencode($relative).'&b='.$_GET['b'].'&p='.$_GET['p'];
	}
	...
	function dir_size($path)    {
        ...
        while($file = readdir($dir))
        {
            ...
            if(is_dir($path."/".$file) && $file!="." && $file !="..")$size += $this->dir_size($path."/".$file);
