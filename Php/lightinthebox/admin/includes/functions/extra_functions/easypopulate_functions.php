  ...
  $products = $db->Execute($sql);
	...
	if (mysql_errno()) {
		...
		if ($ep_debug_logging == true) {
			...
			$string = "MySQL 错误 ".mysql_errno().": ".mysql_error()."\n运行:\n$sql\n";
...
function install_easypopulate() {
	...
	$db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " VALUES 
		(NULL, '上传目录', 'EASYPOPULATE_CONFIG_TEMP_DIR', 'temp/', '上传目录的名称 (缺省: temp/)', " . $group_id . ", '0', NULL, now(), NULL, NULL),
		(NULL, '上传文件日期格式', 'EASYPOPULATE_CONFIG_FILE_DATE_FORMAT', 'y-m-d', '选择上传文件日期格式，通常由MS Excel生成。上传文件的RAW日期 (例如 2005-09-26 09:00:00) 不受影响，会照样上传。', " . $group_id . ", '1', NULL, now(), NULL, 'zen_cfg_select_option(array(\"m-d-y\", \"d-m-y\", \"y-m-d\"),'),
		(NULL, '缺省RAW时间格式', 'EASYPOPULATE_CONFIG_DEFAULT_RAW_TIME', '09:00:00', '如果上传文件中没有指定时间，将使用该值。适合指定特价在某个时间后生效。(缺省: 09:00:00)', " . $group_id . ", '2', NULL, now(), NULL, NULL),
		(NULL, '在#记录后分割文件', 'EASYPOPULATE_CONFIG_SPLIT_MAX', '300', '缺省的分割上传文件的记录数目。用于防止大量上传时出现的超时。(缺省: 300)', " . $group_id . ", '3', NULL, now(), NULL, NULL),
		(NULL, '最大分类深度', 'EASYPOPULATE_CONFIG_MAX_CATEGORY_LEVELS', '7', '商店的最大分类级数。为下载文件中分类栏目的数目(缺省: 7)', " . $group_id . ", '4', NULL, now(), NULL, NULL),
		(NULL, '上传/下载价格含税', 'EASYPOPULATE_CONFIG_PRICE_INC_TAX', 'false', '选择价格是否含税。', " . $group_id . ", '5', NULL, now(), NULL, 'zen_cfg_select_option(array(\"true\", \"false\"),'),
		(NULL, '零数量商品', 'EASYPOPULATE_CONFIG_ZERO_QTY_INACTIVE', 'false', '上传时，是否包含零数量商品。(缺省: false)', " . $group_id . ", '6', NULL, now(), NULL, 'zen_cfg_select_option(array(\"true\", \"false\"),'),
		(NULL, '智能标签行替换', 'EASYPOPULATE_CONFIG_SMART_TAGS', 'true', '上传文件中描述字段，回车符和换行符转换为HTML换行，防止格式错误(缺省: true)', " . $group_id . ", '7', NULL, now(), NULL, 'zen_cfg_select_option(array(\"true\", \"false\"),'),
		(NULL, '高级智能标签', 'EASYPOPULATE_CONFIG_ADV_SMART_TAGS', 'false', '描述字段采用高级格式化，标题加粗、添加列表等。在ADMIN/easypopulate.php文件中设置 (缺省: false)', " . $group_id . ", '8', NULL, now(), NULL, 'zen_cfg_select_option(array(\"true\", \"false\"),'),
		(NULL, '调试记录', 'EASYPOPULATE_CONFIG_DEBUG_LOGGING', 'true', '允许批量商品管理生成错误记录文件 (缺省: true)', " . $group_id . ", '9', NULL, now(), NULL, 'zen_cfg_select_option(array(\"true\", \"false\"),')
		");
}
