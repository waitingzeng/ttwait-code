...
$featured_categories_query = "select c.categories_id,c.categories_image, cd.categories_name
										 ...
										 where c.categories_image != '' and c.categories_id = cd.categories_id and c.categories_type = 2 
...
if ($num_categories > 0) {
  ...
  while (!$featured_categories->EOF and !($row == 2)) {
    ...
    $cPath_top_new = zen_get_path($featured_categories->fields['categories_id']);
    // strip out 0_ from top level cats
    $cPath_top_new = str_replace('=0_', '=', $cPath_top_new);
...
WHERE c1.categories_id=c.categories_id and parent_id = '.$featured_categories->fields['categories_id'] .' limit 5';
		...
		$subCatoryString .= '</dl><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_top_new) . '" class="more_product">More wholesale products ></a>';
		
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<div><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_top_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$featured_categories->fields['categories_name'].'" class="ih">' . zen_image(DIR_WS_IMAGES . $featured_categories->fields['categories_image'], $featured_categories->fields['categories_name'], 148, 113) . '</a></div><h3 class="margin_t line_120 pad_b2"><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_top_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$featured_categories->fields['categories_name'].'">'. $featured_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString);
    $col ++;
