...
$shopping_categories_query = "select c.categories_id,c.categories_image, cd.categories_name
										 ...
										 where c.categories_image != '' and c.categories_id = cd.categories_id and c.categories_type = 3 
...
if ($num_categories > 0) {
  ...
  while (!$shopping_categories->EOF) {
    ...
    $cPath_top_new = zen_get_path($shopping_categories->fields['categories_id']);
...
WHERE c1.categories_id=c.categories_id and parent_id = '.$shopping_categories->fields['categories_id'] .' limit 5';
		...
		$subCatoryString .= '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_top_new) . '" class="more_product">More wholesale products ></a>';
		
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<a href="'. zen_href_link(FILENAME_DEFAULT, $cPath_top_new) . '"  class="ih fl" title="'.SEO_COMMON_KEYWORDS.' '.$shopping_categories->fields['categories_name'].'">' . zen_image(DIR_WS_IMAGES . $shopping_categories->fields['categories_image'], $shopping_categories->fields['categories_name'], 80, 134,' class="fl"') . '</a><div class="block pad_l_90px"><h3><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_top_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$shopping_categories->fields['categories_name'].'">'. $shopping_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString.'</div>');
