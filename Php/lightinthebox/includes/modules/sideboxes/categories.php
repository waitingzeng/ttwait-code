 ...
 */
	    ...
	    $priceListOutString .= '<dt class="b red">Any Price<span class="arrow_r">&nbsp;</span></dt>';
	    for ($i=0; $i<$priceListRowNum; $i++){
	     $priceListOutString .= '<dd>';
	     $priceListOutString .= '<a href="'.zen_href_link(FILENAME_DEFAULT, 'cPath='.$current_category_id.'&min_price='.floor($priceList[sizeof($priceList)*$i/$priceListRowNum])).'&max_price='.floor($priceList[sizeof($priceList)*($i+1)/$priceListRowNum-1]).'" rel="nofollow">';
