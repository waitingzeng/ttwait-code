...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
    ...
    'text' => (($featured_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '"  class="ih" >' . zen_image_OLD(DIR_WS_IMAGES . substr_replace($featured_products->fields['products_image'], 'l/', 0, 2), $featured_products->fields['products_name'], 86, 86) . '</a>') . '<br /><p class="b red g_t_c">' . $products_price .'</p>');
//
/*
