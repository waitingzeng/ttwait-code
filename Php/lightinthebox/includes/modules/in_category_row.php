...
WHERE c1.categories_id=c.categories_id and parent_id = '.$in_categories->fields['categories_id'] .' limit 5';
        ...
        $cPath_new = substr($cPath_new,strrpos($cPath_new,'_')+1);
				$subCatoryString .= '<dt><a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$cPath_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$subCatory->fields['categories_name'].'">'.zen_clipped_string($subCatory->fields['categories_name']).'</a></dt>';
				$subCatory->MoveNext();
		}
		$subCatoryString .= '</dl><a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$in_categories->fields['categories_id']) . '" class="more_product">More wholesale products ></a>';
		*/
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<div><a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$in_categories->fields['categories_id']) . '" title="'.SEO_COMMON_KEYWORDS.' '.$in_categories->fields['categories_name'].'" class="ih">' . zen_image(DIR_WS_IMAGES . $in_categories->fields['categories_image'], $in_categories->fields['categories_name'], 150, 180) . '</a></div><h3 class="margin_t line_120 pad_b2"><a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$in_categories->fields['categories_id']) . '" title="'.SEO_COMMON_KEYWORDS.' '.$in_categories->fields['categories_name'].'">'. $in_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString);
