...
//        break;
     ...
     if (zen_get_faq_types_to_faq_category($box_faq_categories_array[$i]['path']) == '3' or ($box_faq_categories_array[$i]['top'] != 'true' and SHOW_FAQ_CATEGORIES_SUBFAQ_CATEGORIES_ALWAYS != 1)) {
      ...
      $content .= '<li><a class="' . $new_style . '" href="' . zen_href_link(FILENAME_FAQS, $box_faq_categories_array[$i]['path']) . '">';
...
$content .= '</ul>' . "\n";
$content .= '<ul class="pad_1em black big b">Ask a question?';
$content .= '</ul>';
$content .= '<a target="_blank" title="Write an inquiry" href="'.zen_href_link(FILENAME_FAQS).'" class="icon_inquiry center margin_t"> </a>'
?>