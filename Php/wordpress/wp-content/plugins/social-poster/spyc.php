<?php
  /** 
   * Spyc MOD -- A Simple PHP YAML Class
  ...
  class Spyc {
      ...
      foreach ($yaml as $linenum => $line) {
        ...
        if ($this->_inBlock === false && empty($ifchk)) {
          ...
          if ($this->_lastIndent == $node->indent) {
            ...
            if (!is_array($this->_indentSort[$node->indent])) {
                die('Indentation levels are incorrect in configuration file');
            }
    ...
    function _dumpNode($key,$value,$indent) {
      ...
      if (is_int($key)) {
        // It's a sequence
        $string = $spaces.'- '.trim($value)."\n";
      } else {
        // It's mapped
        $string = $spaces.$key.': '.trim($value)."\n";
      }
