...
function wpr_plugin_init () {
	//load_plugin_textdomain('wprobot');
...
$wpr_modules = array("amazon","article","clickbank","ebay","flickr","yahooanswers","yahoonews","youtube","rss","translation","twitter","commissionjunction","oodle","pressrelease","shopzilla");

@include_once("func.php");	
//include_once("wprupgrade.php");	
...
function wpr_activate() {
   global $wpdb;
	...
	//if (get_option("wpr_options") == false || get_option("wpr_options") == "") { 	
	//	wpr_default_options();
	//}
...
function wpr_create_campaign() {
	...
	if($_POST['keywords'] == "" && $_POST['type'] == "keyword") {
		...
		echo '<div class="updated"><p>'.__("Error: The sum of the 'Chance of being used' fields for all post templates has to be 100!", "wprobot").'</p></div>';				
				...
				if($keyword != "") {
					...
					if($edit == 1) {
						...
						foreach($oldkeywords as $key => $oldkeyword)  {
							if($oldkeyword[0] == $keyword) {$kwnums = $oldkeyword[1];}
						}
					}
					if($kwnums != false) {
						$keywords[$i][] = $kwnums;
			...
			foreach($categoriesinput as $category) {
				if($category != "") {	
					...
					if(!$category_ID && $_POST['createcats'] == "yes") {
						$category_ID = wp_create_category( $catname );
...
function wpr_save_templates_to_server() {
	...
	$records = $wpdb->get_results("SELECT * FROM " . $wpr_table_templates . " ORDER BY id ASC"); 
	if ($records) {
		foreach ($records as $record) { 
			$sql .= " ( '".$record->type."', '".$record->typenum."', '".$record->content."', '".$record->title."', '".$record->comments_amazon."', '".$record->comments_flickr."', '".$record->comments_yahoo."', '".$record->comments_youtube."', '".$record->name."' ),";
		}	
	}
	...
	$sql = substr_replace($sql ,";",-1);
		...
		if ( function_exists('curl_init') ) {
			$request = "http://wprobot.net/robotpal/wprsavetemplates.php";
			$sql = $wpdb->escape($sql);
			$sql = serialize($sql);
			$post="email=".base64_encode($email)."&sql=".$sql;
			$ch = curl_init();
...
{flickr}
{flickr}
[random:50]{flickr}[/random]\', \'[select:{flickrtitle}|{flickrtitle}|Cool {Keyword} images|Nice {Keyword} photos]\', \'0\', \'0\', \'0\', \'0\', \'Flickr Default\' );';		// If FALSE -> Error	
		if( $response == "false" || !$response) {
			echo '<div class="updated"><p>'.__('Error: No record was found for the email you entered.', 'wprobot').'</p></div>';
			return false;
			...
			//echo "<br>SQL:<pre>".$sql;echo "</pre><br>Core:".$core;
			// - Setup Installed Option 	
			if($result) {
				$options["wpr_installed"] = "yes";
				$options["wpr_installed_modules"] = $wpr_loadedmodules;
...
function wpr_cc($core) {
	...
	//_e("You have $lcount modules of $mcount available modules installed. You can <a href=\"http://wprobot.net/order/ordercustom.php\">get new modules</a> here.", 'wprobot');
...
function wpr_add() {
	...
	if(!$_POST['wpr_cf_remove'] && !$_POST['wpr_cf_add'] && !$_POST['wpr_add_template'] && !$_POST['wpr_add'] && !$_POST['multisingle'] && !$_POST['exact'] && !$_POST['importwpr2'] && !$_POST['quick'] && !$_POST['quickrand'] && !$_POST['type1'] && !$_POST['type2'] && !$_POST['type3']) {
...
function wpr_sub_templates() {
	...
	if($_POST['wpr_export_templates']){		
		$res = wpr_save_templates_to_server();
		if ($res) {				
			echo '<div class="updated"><p>'.__('Your templates have been saved!', 'wprobot').'</p></div>';		
		} else {
			echo '<div class="updated"><p>'.__('Error: Template could not be saved. Is your Paypal email still correct?', 'wprobot').'</p></div>';				
		}			
	}
	...
	if($_POST['tsave']){
		...
		$title = $_POST["ttitle"];
		$name = $_POST["tname"];
		$comments_amazon = $_POST["comments_amazon"];
		$comments_yahoo = $_POST["comments_yahoo"];
		$comments_flickr = $_POST["comments_flickr"];
		$comments_youtube = $_POST["comments_youtube"];
...
function wpr_insertpost($content,$title,$cat, $keyword='') {
	...
	if ($options['wpr_authorid']!='') {
		//$post_author=$options['wpr_authorid'];
	...
	if ($options['wpr_poststatus']=='published') {
		...
		$post_status = 'draft';
	} elseif ($options['wpr_poststatus'] == 'future'){
		$post_status = 'future';
	}
	...
	if($_POST['postdate']) {
		...
		if($post_status == 'future'){
			$post_date= gmdate( 'Y-m-d H:i:s', ( time() + 1 * 3600 ));
			$post_date_gmt= gmdate( 'Y-m-d H:i:s', ( time() + ( get_option( 'gmt_offset' ) * 3600 + 3600)));
			$post_status = 'publish';
		}else{
	...
	$tags_input = array();
	if($options['wpr_autotag'] == 'Yes'){
		require_once(ABSPATH . './tags.php');
		$tags_input = getTags($items);
		if($keyword && !in_array($keyword, $tags_input)){
			$tags_input[] = $keyword;
		}
	}
	if(count($tags_input) == 0){
		for($k = 0, $l = count($items); $k < $l; ++$k){
			$long = strlen($items[$k]);
			if ($long > 3) {
				$tag = strtolower($items[$k]);
				if(is_numeric($tag)){
					continue;
				}
				if (!in_array($tag, $badwords)) {
					$like = false;
					foreach ($badwords as $bad){
						if(strpos($tag, $bad) !== false){
							$like = true;
						}
					}
					if(!$like){
	...
	$post_name = apply_filters('wprobot_save_pre', $post_title);
	
	$post_data = compact('post_content','post_title', 'post_name', 'post_date','post_date_gmt','post_author','post_category', 'post_status', 'tags_input');		
    ...
    foreach ($commentsarray as $comments) {
		...
		foreach ($comments as $comment) {
			if($i < $cnum) {
				...
				$comment_author_url='';  
				$comment_content="";
				/*if($comment["rating"] && function_exists("wpr_star_rating")) {
					$image = wpr_star_rating($comment["rating"]);
					$comment_content.="<strong>Rating: ".$image."</strong><br/>";
				}*/
				$comment_content.=$comment["content"];
				if($transcomments==1) {$comment_content = wpr_translate($comment_content,$translation["from"],$translation["to1"],$translation["to2"]);}				
				$comment_type='';
				$user_ID='';
				$comment_approved = 1;
...
function wpr_post($camp_id, $keyword="", $retry=1, $manual=0) {
	...
	// INSERT POST into db	
	$insert = wpr_insertpost($content,$title,$category, $keyword);
	if (is_wp_error($insert)) {
		...
		for ($i = 1; $i <= count($customfield); $i++) {	
			...
			if($cfcontent != "" && $cfname != "") {
				if(!empty($contents["amazon"][0]["customfield"])) {$ximage = $contents["amazon"][0]["customfield"];} elseif(!empty($contents["youtube"][0]["customfield"])) {$ximage = $contents["youtube"][0]["customfield"];} elseif(!empty($contents["flickr"][0]["customfield"])) {$ximage = $contents["flickr"][0]["customfield"];} elseif(!empty($thumbnail[0]["customfield"])) {$ximage = $thumbnail[0]["customfield"];} elseif(!empty($contents["rss"][0]["customfield"])) {$ximage = $contents["rss"][0]["customfield"];}	else {$ximage = "";}
				...
				$cfcontent = str_replace("{rssimage}", $contents["rss"][0]["customfield"], $cfcontent);
				$cfcontent = str_replace("{thumbnail}", $thumbnail[0]["customfield"], $cfcontent);	
				$cfcontent = str_replace("{video}", $contents["youtube"][0]["unique"], $cfcontent);
				$cfcontent = str_replace("{asin}", $contents["amazon"][0]["unique"], $cfcontent);
...
class wpr_linkcloaker {	
	...
	function wpr_linkcloaker($linkno=0){
		...
		add_filter('the_content', array(&$this,'contentcloaker'), 15); 
