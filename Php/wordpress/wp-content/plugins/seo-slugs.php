...
add_filter('wprobot_save_pre', 'seo_slugs_func', 0);

function seo_slugs($slug) {
    // We don't want to change an existing slug
	if ($slug) return $slug;

	return seo_slugs_func($_POST['post_title']);
}

function seo_slugs_func($title){
	$seo_slug = strtolower(stripslashes($title));
