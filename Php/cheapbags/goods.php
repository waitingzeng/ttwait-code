...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        /* current position */
        $page_title = htmlspecialchars($goods['keywords']);
        if(!$page_title) $page_title = $position['title'];
        ...
        if(empty($pictures)){
            $pictures[0]['img_url'] = $goods['goods_img'];
            $pictures[0]['thumb_url'] = $goods['goods_thumb'];
        }
