...
$rss->addDCdata('', 'http://cheapbagswholesale.com', date('r'));
...
else
{
    ...
    $sql = 'SELECT c.cat_name, g.goods_id, g.goods_name, g.keywords, g.last_update ' .
    ...
    if ($res !== false)
    {
        while ($row = $db->fetchRow($res))
        {
            ...
            $title    = htmlspecialchars($row['goods_name']);
            $link     = $uri . $item_url; #. $separator . 'from=rss';
            $desc     = htmlspecialchars($row['keywords']);
