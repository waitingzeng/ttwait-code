		...
		private int checkLongOption()
		{
			...
			if (ambig && !exact)
			{
				if (this.opterr)
				{
                    ...
                    System.Console.Error.WriteLine("{0}: option ''{1}'' is ambiguous", msgArgs);
				}
				...
				else if (pfound.HasArg == Argument.Required)
				{
					...
					else
					{
						if (this.opterr)
						{
							...
							System.Console.Error.WriteLine(
                                "{0}: option ''{1}'' requires an argument", msgArgs);
						}
					...
					|| (this.optstring.IndexOf(this.nextchar[0]) == - 1))
				{
					if (this.opterr)
					{
						if (this.argv[this.optind].StartsWith("--"))
						{
							...
							System.Console.Error.WriteLine(
                                "{0}: unrecognized option ''--{1}''", msgArgs);
						}
						else
						{
							object[] msgArgs = new object[]{ this.progname,
							...
							System.Console.Error.WriteLine(
                                "{0}: unrecognized option ''{1}{2}''", msgArgs);
						}
					}
